...
## Non-Normative Clarification A: Authority vs Compute

In SER-FED, authority is a function of verified history (Resolution Notes, RN),
bounded by role/domain scope, time decay, and bonded liability.

Compute capacity (including access to stronger models or oracles)
never confers authority directly.
It only affects execution capability and economic cost.

---

## Non-Normative Clarification B: Challenge Window v0.2 (Two-Channel Immune Model)

The Challenge Window operates as an immune mechanism, not a judicial one.
Its role is to prevent silent authority accumulation while reality (L4) is still unfolding.

Channel A — Algorithmic Signal (“Smell of Error”):
Low-cost, high-frequency anomaly detection
(strategy drift, telemetry mismatch, budget envelope violations).
Effect: Pre-Challenge Hold (authority growth frozen, no punishment).

Channel B — Bonded Challenge (“Skin in the Game”):
Any eligible participant may escalate by staking a Challenge Bond.
Effect: Opens full dispute resolution with OP/CR collection.

Convergence Rule:
Escalation requires both an algorithmic signal (A)
and an economic commitment (B), or explicit High-Impact classification.

Early uncertainty does not imply guilt.
Resolution Notes may be:
UNCONFIRMED, PARTIALLY_CONFIRMED, or REFUTED.
REFUTED requires convergent OP/CR evidence and L4 contradiction.
